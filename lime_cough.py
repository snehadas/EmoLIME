...
class CoughExplanation(object):
    ...
    def image_spectral(self, label, positive_only=True, negative_only=False, hide_rest=True, num_features=5,
        ...
        plt.imshow(marked[:, :, 2], origin="lower", cmap=plt.get_cmap("gray")) # magma to gray
        plt.colorbar(format='%+2.0f dB') # show colorbar

        if show_colors:
            normalized_weights = self.normalize(weights)

            # write weights to a file
            with open('spectral_weights.csv', 'a', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(weights)
            print('Weights written to file')  # If needed for debug
            ...
            for index, comp in enumerate(indices_comp):
                ...
                # Determine the position for the label, assuming center of the mask for simplicity
                center_y, center_x = np.mean(np.where(mask), axis=1)
                plt.text(center_x, center_y, f'{weights[index]:.2f}', color='white', ha='center', va='center', fontsize = 14, fontweight='bold')
        ...
        tick_labels = np.linspace(0, sr / 2000, num_ticks).astype(int)
        print(tick_labels)
    ...
    def image_temporal(self, label, positive_only=True, negative_only=False, hide_rest=False, num_features=3, 
        ...
        ax1.text(2, 1.3, f'Weights', #
                        horizontalalignment='center', verticalalignment='center',  #
                        color='black', fontsize=12, style='italic')

        if show_colors:
            normalized_weights = self.normalize(weights)
            cmap = plt.get_cmap('seismic')  # Choose a visually appealing colormap
            norm = Normalize(vmin=-0.3, vmax=0.3)#vmin=min(weights), vmax=max(weights)) #
            mappable = ScalarMappable(norm=norm, cmap=cmap) #
            mappable.set_array(weights) #
            
            for index, comp in enumerate(indices_comp): #
                color = cmap(norm(weights[index])) #
                ax1.axvspan(indices[comp], indices[comp+1], facecolor=color, alpha=0.5)  # Use normalized opacity #

                # Adding text label inside each span #
                text_pos_x = (indices[comp] + indices[comp+1]) / 2 #
                text_pos_y = 1.1  # adjust y position based on your plot scale #
                ax1.text(text_pos_x, text_pos_y, f'{weights[index]:.2f}', #
                        horizontalalignment='center', verticalalignment='center',  #
...
class LimeCoughExplainer(object):
    ...
    def __init__(self, kernel_width=.25, kernel=None, verbose=False,
                 feature_selection='auto', random_state=42): # set random seed!
        """
        Init function
